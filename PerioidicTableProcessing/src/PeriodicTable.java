...
public class PeriodicTable extends PApplet implements WindowListener {
	...
	url.addMouseListener(new MouseAdapter() {
		...
		Color c = elementInfo.get("cpk-hex").toString().equals("null") ? Color.CYAN.darker() : Color.decode("#" + elementInfo.get("cpk-hex").toString().toUpperCase()); 
		url.setForeground(c);
	    }
	...
	for (ElementComponent c : components) {
	    ...
	    c.changeColor(s.color[0], s.color[1], s.color[2]);
	}
    }
